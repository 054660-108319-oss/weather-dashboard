...
function generateOutfitRecommendation(temp, status) {
    ...
    // 🌟 每次查詢前，先清除舊的背景主題
    body.classList.remove('hot-theme', 'comfortable-theme', 'cold-theme');

    // 🌟 依據溫度/天氣，動態為 body 加上對應的 CSS 樣式
    if (status === "Rain" || status === "Thunderstorm" || temp < 15) {
        body.classList.add('cold-theme'); 
    } else if (temp >= 15 && temp <= 28) {
        body.classList.add('comfortable-theme'); 
    } else if (temp > 28) {
